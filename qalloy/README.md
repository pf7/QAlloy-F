...
QAlloy-F is an extension of Alloy with *fuzzy relations*, following the previous work of developing an extension to Alloy to quantitative relations, namely in the integer domain. 

This enables QAlloy to tackle fuzzy problems through fuzzy relational reasoning, allowing for high-level quantitative relational models in the fuzzy realm. The QAlloy-F Analyzer also implements an SMT-based engine to effectively reason over real-numbered quantities from the unit interval.

The integer component of QAlloy can be found at: https://github.com/pf7/QAlloy
...
The QAlloy-F build is using a _bnd workspace_ setup using a maven layout. This means it can be build  with Gradle and  the Eclipse IDE for interactive development. Projects are setup to continuously deliver the executable.
